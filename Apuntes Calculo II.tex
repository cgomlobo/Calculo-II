...
-\underline{Definición:} Una \textbf{sucesión en} $\mathbf{\R^n}$ es una colección de n sucesiones en $\R 
: \{A_k\}_{k \in \N} = \{(a_1^k, a_2^k,..., a_n^k)\}_{k \in \N}$.
\vspace{3mm}

\indent \underline{Ejemplos:} $\{A_k\}_{k \in \N} = \left\{ \left (k, \bfrac{1}{k^2}, e^{-k}\right ) \right \} 
_{k \in \N}; \{A_k\}_{k \in \N} = \left \{ \left (\bfrac{3k^2}{k^2-1}, \bfrac{1}{k}, \log \bfrac{k+1}{k} \right 
)\right \}_{k \in \N}$.\vspace{5mm}

\noindent-\underline{Definición:} Dada una sucesión $\{A_k\}_{k \in \N} \in \R^n$ y un punto $L \in \R^n$, 
