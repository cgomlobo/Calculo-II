...
\teorema{Propiedades de los límites} Sean $f, g : A \text{(abierto)} \subset \R^n \to \R$, $\displaystyle
\lim_{x \to x_0} f (x) = L_1 \text{ y } \lim_{x \to x_0} g (x) = L_2$:
\vspace{3mm}
...
\teorema[Proposición 1] Sea $f : A \text{ (abierto) }  \subset \R^n \rightarrow \R^m \text{ una función 
vectorial , } x_0 \in A' \text{ y } L \in L \in \R^m$, entonces:
\[
\boxed{
...
{\color{red}\teorema[Proposición 2] "Caracterización de límites de funciones por  sucesiones"}
